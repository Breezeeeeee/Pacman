...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    if manhattanDistanceToGhost > 5 or len(invaders)==0:
      self.refreshPassedPositionWeight(gameState)
  ...
  def getFeatures(self, gameState, action):
    ...
    if scaredTimes>3:
      ...
      if len(invaders) > 0:
        ...
        if self.passedPositionWeight[myPos] > 3:
          features['repeatPosition'] = self.passedPositionWeight[myPos]

      if distanceToGhost < 4+self.negetiveReward*2:
  ...
  def getWeights(self, gameState, action):
    ...
    if foodLeft<=2:
      return {'successorScore': 0, 'distanceToFood': 0,
              ...
              'meetGhost':-1000000,'reverse': -2,'repeatPosition':-300,'distanceToCapsule':1}
    return {'successorScore': 100, 'distanceToFood': -2,
            'distanceToGhost':100, 'distanceToHome':60,'inCorner':-100000,
            'meetGhost':-1000000 ,'reverse': -2, 'distanceToCapsule':1,'repeatPosition':-300}
