...
class OffensiveReflexAgent(ReflexCaptureAgent):
    ...
    def getGhostPotentialLocation(self, ghostPos, gameState):
        newGhostPos = []
        for pos in ghostPos:
            if not gameState.hasWall((int)(pos[0] + 1), (int)(pos[1])) and (pos[0] + 1, pos[1]) not in ghostPos and (pos[0] + 1, pos[1]) not in newGhostPos:
                newGhostPos.append((pos[0] + 1, pos[1]))
            if not gameState.hasWall((int)(pos[0] - 1), (int)(pos[1])) and (pos[0] - 1, pos[1]) not in ghostPos and (pos[0] - 1, pos[1]) not in newGhostPos:
                newGhostPos.append((pos[0] - 1, pos[1]))
            if not gameState.hasWall((int)(pos[0]), (int)(pos[1] + 1)) and (pos[0], pos[1] + 1) not in ghostPos and (pos[0], pos[1]+1) not in newGhostPos:
                newGhostPos.append((pos[0], pos[1] + 1))
            if not gameState.hasWall((int)(pos[0]), (int)(pos[1] - 1)) and (pos[0], pos[1] - 1) not in ghostPos and (pos[0] + 1, pos[1]-1) not in newGhostPos:
                newGhostPos.append((pos[0], pos[1] - 1))
        for pos in ghostPos:
            newGhostPos.append(pos)
        return newGhostPos
    def getSuccessorWithGhost(self,gameState,ghostPos,action):
        ghostPos=self.getGhostPotentialLocation(ghostPos,gameState)
    def chooseAction(self, gameState):
        """
        Picks among the actions with the highest Q(s,a).
        """
        ...
        finalAction = self.NStepsStart(gameState,ghostPos,5)
        ...
        if len(currentFoodList) > len(successor):
            ...
            self.refreshPassedPositionWeight(gameState)
        return finalAction
    def NSteps(self,gameState,ghostPos,step):
        reward=util.Counter()
        actions = gameState.getLegalActions(self.index)
        ghostPosCopy = self.getGhostPotentialLocation(ghostPos, gameState)
        ...
        if step>1:
            for action in actions:
                ...
                if myPos in ghostPos:
                    return -1  # 如果进入ghost可能存在的地方，那该位置定位错误
                successorFood = self.getFood(successor).asList()
                currentFoodList = self.getFood(gameState).asList()
                if len(currentFoodList) > len(successorFood):
                    # 吃到food才给reward

                    reward[action] = self.NSteps(successor,ghostPosCopy,step-1)+1
                else:
                    reward[action] = self.NSteps( successor,ghostPosCopy, step - 1)

            value=reward[max(reward)]
            ...
            return value
    def NStepsStart(self,gameState,ghostPos,step):
        reward=util.Counter()
        actions = gameState.getLegalActions(self.index)
        ghostPosCopy = self.getGhostPotentialLocation(ghostPos, gameState)
        ...
        if step>1:
            for action in actions:
                ...
                myPos = successor.getAgentState(self.index).getPosition()
                if myPos in ghostPos:
                    reward[action]=-1  # 如果进入ghost可能存在的地方，那该位置定位错误
                successorFood = self.getFood(successor).asList()
                currentFoodList = self.getFood(gameState).asList()
                if len(currentFoodList) > len(successorFood):
                    # 吃到food才给reward
            ...
            print("ghost position is:",ghostPosCopy)
            value=reward[max(reward)]
            if value==0:#如果都没有给reward，就直接使用approximate Q value
                values = [self.evaluate(gameState, a) for a in actions]
                # print 'eval time for agent %d: %.4f' % (self.index, time.time() - start)
            ...
            if value == 0:
                ...
                maxValue = max(values)
                self.actionValues.append(maxValue)
                bestActions = [a for a, v in zip(actions, values) if v == maxValue]
                finalAction = random.choice(bestActions)
            else:
                finalAction = max(reward)
