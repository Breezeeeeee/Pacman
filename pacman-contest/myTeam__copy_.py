...
class OffensiveReflexAgent(ReflexCaptureAgent):
  ...
  def getFeatures(self, gameState, action):
    ...
    if scaredTimes>3:
      ...
      if distanceToGhost < 6+self.negetiveReward or foodLeft<=2:
  ...
  def getWeights(self, gameState, action):
    ...
    return {'successorScore': 100, 'distanceToFood': -2,
            'distanceToGhost':-100, 'distanceToHome':-60,'inCorner':-100000,'finalDistanceToHome':0, 'distanceToCapsule':-1}
...
class DefensiveReflexAgent(ReflexCaptureAgent):
  ...
  lastLostFoodEffect = 15 #use to measure the effective of last lost food position
  def getFeatures(self, gameState, action):
    ...
    if len(invaders)==0:
      ...
      if len(ghosts)>0:
        for a in invaders:
          if distanceCalculator.manhattanDistance(myPos, a.getPosition())<5:
            features['distanceToCenter']+=abs(myPos[1]-a.getPosition()[1])
