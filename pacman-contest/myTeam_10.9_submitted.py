...
class OffensiveReflexAgent(ReflexCaptureAgent):
    ...
    def getDistanceToTop(self, gameState, myPos):
        ...
        nearest = height
        for location in centerList:
            if abs(location[1] < nearest):
                centerLocation = location
                nearest = abs(location[1]< nearest)
        return self.getMazeDistance(myPos, centerLocation)
    def getFeatures(self, gameState, action):

        if self.repeatActionDetect() and self.goOffensive :
            if self.getScore(gameState) > 0:
                self.goOffensive=False
            else:
                self.escapeFlag=10
        if self.getScore(gameState) < 0:
            self.goOffensive = True
        if self.escapeFlag>0:
            ...
            if scaredTimes > 3:
                ...
                if distanceToGhost < 12 + self.negetiveReward:
